...
*	
* Add Function:
* 14. uint32_t OLED_Pow(uint32_t X, uint32_t Y)
...
// Calls          : 
// Parameters     : none
// Description    : 将OLED 从休眠中唤醒 
//--------------------------------------------------------------
void OLED_ON(void)
{
	WriteCmd(0X8D);  // 设置电荷泵
	WriteCmd(0X14);  // 开启电荷泵
	WriteCmd(0XAF);  // OLED 唤醒
}
...
// Calls          : 
// Parameters     : none
// Description    : 让OLE休眠 -- 休眠模式下，OLED功耗不到10uA
//--------------------------------------------------------------
void OLED_OFF(void)
{
	WriteCmd(0X8D);  // 设置电荷泵
	WriteCmd(0X10);  // 关闭电荷泵
	WriteCmd(0XAE);  //OLED 休眠
}
...
/**
* @brief Add Func : 显示codetab.h中的ASCII字符,有 6*8 和 8*16 可选择
...
void OLED_ShowNum(unsigned char x, unsigned char y, uint32_t Num,uint8_t Length, unsigned char TextSize)
{
	...
	switch(TextSize)
	{
		case 1:
		{
			...
			//利用 单个数字+'0'显示字符， 即利用OLED_ShowChar来显示数字
			for(idx = 0;idx < Length;idx++)
			{
				// 6个像素作为一个字符宽度，所以下一个数字需要将位置向右移动6像素的位置
				OLED_ShowChar(x+idx*6, y, Num / OLED_Pow(10, Length - idx - 1) % 10 + '0',TextSize);
			}
		}break;
		case 2:
		{
			...
			for(idx = 0;idx < Length;idx++)
			{
				OLED_ShowChar(x + idx*8, y , Num / OLED_Pow(10, Length - idx - 1) % 10 + '0',TextSize);
			}
...
void OLED_ShowSignedNum(unsigned char x, unsigned char y, int32_t Num,uint8_t Length, unsigned char TextSize)
{
	...
	switch(TextSize)
	{
		case 1:
		{
			...
			{
				// 6个像素作为一个字符宽度，所以下一个数字需要将位置向右移动6像素的位置
				OLED_ShowChar(x+idx*6, y, Number / OLED_Pow(10, Length - idx - 1) % 10 + '0',TextSize);
			}
		...
		case 2:
		{
			...
			for(idx = 0;idx < Length;idx++)
			{
				OLED_ShowChar(x + idx*8, y , Number / OLED_Pow(10, Length - idx - 1) % 10 + '0',TextSize);
			}
		...
		case 1:
		{
			...
			if(y<8)
			{
				for(i = 0;i<Length;i++)
				{
					...
					if (SingleNum < 10)
					{
						OLED_ShowChar(x+i*6, y, SingleNum + '0',TextSize);
					}
					else
					{
						OLED_ShowChar(x+i*6, y, SingleNum - 10 + 'A',TextSize);
					}	
				}
			}
		...
		case 2:
		{
			...
			if(y<8){
				for(i = 0;i<Length;i++)
				{
					...
					if (SingleNum < 10)
					{
						OLED_ShowChar(x+i*8, y, SingleNum + '0',TextSize);
					}
					else
					{
						OLED_ShowChar(x+i*8, y, SingleNum - 10 + 'A',TextSize);
					}
				}
	...
	switch(TextSize){
		case 1:
		{
			...
			if(y<8){
				for(i = 0;i<Length;i++)
				{
					SingleNum = Num / OLED_Pow(2,Length - i - 1) %2;
					OLED_ShowChar(x+i*6, y, SingleNum + '0',TextSize);
				}
			}
